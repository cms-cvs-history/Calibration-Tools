...
//  std::cerr << "[PG] IMAEBphiWidth = " << IMAEBphiWidth << std::endl ;
  ...
  std::map<int, EcalIntercalibConstants::EcalIntercalibConstantMap> recalibrators ; 
...
//      std::cerr << "PG nomefile: " << fileName << std::endl ;
  ...
  for (std::map<int, EcalIntercalibConstants::EcalIntercalibConstantMap>::const_iterator itMap = 
          recalibrators.begin () ;
      ...
      for (int iphi = IMAEBphiStart ; iphi < IMAEBphiEnd ; ++iphi)
        {
          ...
          std::cerr << "[INFO] writing " << ieta << " " << iphi 
                    << " " << itMap->second.find (det.rawId ())->second 
                    << std::endl ;
          barrelWriter.writeLine (det,itMap->second.find (det.rawId ())->second);
